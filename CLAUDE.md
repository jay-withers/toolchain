...
Hooks are in `.pre-commit-config.yaml` at the repo root (pre-commit's default location, so no `--config` flag is needed). The `no-commit-to-branch` hook blocks direct commits to `main`.
